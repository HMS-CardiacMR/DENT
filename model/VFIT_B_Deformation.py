...
class UNet_3D_3D(nn.Module):
    ...
    def forward(self, frames):
        ...
        out_ll, _ = self.predict_ll(fea3, frames, x_2.size()[-2:])

        out_l, _ = self.predict_l(fea2, frames, x_1.size()[-2:])
        out_l = F.interpolate(out_ll, size=out_l.size()[-2:], mode='bilinear') + out_l

        out, deformation = self.predict(fea1, frames, x_0.size()[-2:])
...
class SynBlock(nn.Module):
    ...
    def forward(self, fea, frames, output_size):
        ...
        deformation = weights, alphas, betas, Occlusion
